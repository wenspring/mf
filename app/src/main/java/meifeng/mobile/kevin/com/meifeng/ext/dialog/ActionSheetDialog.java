...
public class ActionSheetDialog {
	...
	public ActionSheetDialog builder() {
		...
		View view = inflater.inflate(R.layout.view_action_sheet, new LinearLayout(mContext), false);
	...
	public ActionSheetDialog setTitleTextSize(float size) {
		txt_title.setTextSize(size);
		return this;
	}

	public ActionSheetDialog setTitleTextColor(int color) {
		txt_title.setTextColor(color);
		return this;
	}

	public ActionSheetDialog setTitleTextColor(ColorStateList colors) {
	...
	private void setSheetItems() {
		...
		if (size >= 7) {
			...
			int height = DensityUtil.getHeightInPx(mContext);
		...
		for (int i = 1; i <= size; i++) {
			...
			if (size == 1) {
				if (showTitle) {
					textView.setBackgroundResource(R.drawable.selector_actionsheet_bottom_);
				} else {
					textView.setBackgroundResource(R.drawable.selector_actionsheet_single_);
				}
			} else {
				if (showTitle) {
					if (i >= 1 && i < size) {
